...
SumStatsDA1 <- dplyr::summarise(group_by(percent_da, trt, DA_responsive), n = n(), mean_perc = mean(perc_inc),sd_perc = sd(perc_inc), se_perc = sd_perc/sqrt(n),
)
SumStatsDA1

plot_da_perc <-  ggplot() + geom_bar(data= SumStatsDA1, aes(DA_responsive, mean_perc, fill= DA_responsive), position= "dodge", stat = "identity", width = 0.5) + geom_point(data=percent_da, aes(x= DA_responsive, y=perc_inc), size=2) + geom_errorbar(data= SumStatsDA1, aes(x=DA_responsive, ymin=mean_perc-se_perc, ymax=mean_perc+se_perc, fill = DA_responsive), position=position_dodge(width=0.9), width=.25) + scale_y_continuous(labels = scales::percent, limits = c(-0.1,0.8)) + theme_bw() + ylab("Percent Peak Current Change")
plot_da_perc
...
#percent_skf <- skf_response %>% group_by(cell, DA_responsive) %>% dplyr::summarise(base = (amplitude[treatment=="control"] + amplitude[treatment=="SCH39166"])/2, perc_inc = (amplitude[treatment=="SKF38393"]/ base)-1)
percent_skf$trt <- 'skf' 
percent_skf
...
pwr.t.test(d=-3.596382,power=0.9,sig.level=.05,type="two.sample",alternative="less")
```
...
response_histo2 <- ggplot(percent_dopamine, aes(x=percent, color=DA_responsive, fill= DA_responsive)) + geom_histogram(aes(y=..density..), binwidth=1.5, alpha=0.2, position="identity") + geom_density(alpha=0.6) + geom_vline(data=SumStatsDopamineR, aes(xintercept=mean_perc, color=DA_responsive), linetype="dotted", size=1.5) + geom_vline(xintercept =12.2, linetype="dashed", size=0.7) 
...
response_histo3 <- ggplot(percent_dopamine, aes(x=percent, color=DA_responsive, fill= DA_responsive)) + geom_histogram(aes(y=..density..), binwidth=1.5, alpha=0.2, position="identity") + geom_density(alpha=0.6) + geom_vline(data=SumStatsDopamineR, aes(xintercept=mean_perc, color=DA_responsive), linetype="dotted", size=1.5) + 
  annotate('rect', xmin=-15, xmax=12.2, ymin=0, ymax=Inf, alpha=.2, fill='red')
